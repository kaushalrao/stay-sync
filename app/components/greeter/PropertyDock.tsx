...
export const PropertyDock: React.FC<{
    ...
    disabled?: boolean;
}> = ({ properties, selectedId, onSelect, disabled }) => (
    <div className={`flex justify-center sticky top-2 z-40 w-full pointer-events-none ${disabled ? 'opacity-70' : ''}`}>
        <div className="pointer-events-auto inline-flex overflow-x-auto gap-2 p-1.5 bg-white/95 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-300 dark:border-slate-700 rounded-full shadow-xl no-scrollbar max-w-[calc(100vw-24px)] md:max-w-full">
            {properties.map(p => (
                <button
                    ...
                    disabled={disabled}
                    className={`relative flex-shrink-0 whitespace-nowrap px-4 py-2 rounded-full text-xs font-bold transition-all duration-200 flex items-center gap-1.5 ${disabled ? 'cursor-not-allowed' : 'active:scale-95'} ${selectedId === p.id
