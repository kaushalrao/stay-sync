    ...
    return (
        <button
            ...
            className={`
                ...
                ${getRoomGradient(room, isDone)}
                ${isDone ? 'shadow-lg shadow-emerald-500/20 border-transparent' : 'bg-white dark:bg-slate-800/50 shadow-sm hover:shadow-md'}
            `}
            style={{ animationDelay: `${idx * 100}ms` }}
        >
            <div className="flex justify-between items-start mb-4">
                <div className={`p-3 rounded-2xl ${isDone ? 'bg-white/20 text-white' : 'bg-white dark:bg-slate-800 shadow-sm'}`}>
                    {getRoomIcon(room)}
                </div>
            ...
            <p className={`text-sm mb-4 ${isDone ? 'text-emerald-50' : 'text-slate-500 dark:text-slate-400'}`}>
            ...
            <div className="h-2 w-full bg-black/5 dark:bg-white/10 rounded-full overflow-hidden">
                <div
                    className={`h-full rounded-full transition-all duration-500 ${isDone ? 'bg-white' : 'bg-current opacity-80'}`}
