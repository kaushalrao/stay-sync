        ...
        >
            ...
            <div className="absolute top-3 left-3 md:top-8 md:left-8 z-10">
                <div className={`${styles.labelBg} w-fit px-2 py-0.5 md:px-3 md:py-1 rounded-full backdrop-blur-md mb-2 md:mb-4 border border-white/5`}>
                    <span className={`${styles.labelText} text-[8px] md:text-xs font-bold tracking-wider uppercase`}>{label}</span>
                </div>
                <h2 className="text-lg md:text-4xl font-black text-white mb-1 md:mb-3 tracking-tighter leading-tight">{title}</h2>
            ...
            <div className={`absolute -bottom-2 -right-2 md:-bottom-8 md:-right-8 transition-transform duration-500 group-hover:scale-110 ${iconRotation} ${iconHoverRotation}`}>
                <Icon className={`w-16 h-16 md:w-[180px] md:h-[180px] ${styles.icon} mix-blend-screen drop-shadow-2xl opacity-90`} strokeWidth={1.5} />
            </div>
        </Link>
    );
};
