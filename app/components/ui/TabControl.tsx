...
export const TabControl: React.FC<TabControlProps> = ({ options, activeId, onChange, className = '' }) => (
    <div className={`bg-slate-200/90 dark:bg-slate-900/90 p-1 rounded-2xl inline-flex border border-slate-300 dark:border-white/10 relative shadow-xl w-full sm:w-auto ${className}`}>
        {options.map(opt => (
            <button
                key={opt.id}
        ...
        <div
            className="absolute top-1 bottom-1 rounded-xl bg-gradient-to-br from-slate-700 to-slate-800 dark:from-slate-700 dark:to-slate-800 shadow-lg transition-all duration-300 ease-out"
            style={{
