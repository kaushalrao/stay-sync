...
export const StatCard: React.FC<StatCardProps> = ({ label, value, icon: Icon, color, bg, border }) => {
    return (
        <div className={`relative overflow-hidden rounded-[1.5rem] md:rounded-[2rem] p-4 md:p-6 bg-white dark:bg-slate-900/40 backdrop-blur-xl border border-slate-200 dark:border-white/5 shadow-xl md:shadow-2xl dark:shadow-xl group hover:scale-[1.02] transition-all duration-300`}>
            <div className="absolute inset-0 opacity-0 dark:opacity-20 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] mix-blend-overlay"></div>
